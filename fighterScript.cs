...
public class fighterScript : MonoBehaviour
{
    ...
    [Tooltip("loot item")] public GameObject itemPrefab;        // droped item from enemy
    ...
    [Tooltip("speed of punching")] public float hitTime;        //how long fighter is demobilized because of hitting
    ...
    [HideInInspector]
    public bool dead; // shows if fighter is dead
    ...
    void FixedUpdate()
    {
        ...
        if (!underControl && !demobilized && !dead)
        { // checks if fighter is alive and not under player's control
    ...
    {
        if (!demobilized && !dead && !isBlocking)
        { // checks if fighter is alive, not demobilized and not blocking
            rb.velocity = new Vector2(x, rb.velocity.y / speed) * speed; // applies horizontal movement
            if (grounded)
            { // checks if fighter is grounded
                rb.velocity = new Vector2(rb.velocity.x / speed*1.5f, y) * speed; // applies vertical movement
                if (rb.velocity == Vector2.zero)
                { // checks if fighter is standing
                    an.SetInteger("movingState", 0); // sets idle animation
                    CancelInvoke("run"); // disables invoking run animation
                    invoked = false;
                }
                else
                { // if fighter is moving
                    if (!invoked)
                    { // checks if invoke is not called yet
                        an.SetInteger("movingState", 2);            // was changed from 1 !!!!!!!! 1 is walking, 2 is running
    ...
    public void idle()
    {
        ...
        rb.gravityScale = 0;   // check gravity just in case (had bug with falling after instant damage)
    }
    ...
    void checkForEnemy() // checks for enemy around
    {
        ...
        { 
            ...
            { // check other enemies if some of them are closer
            ...
            if (Vector3.Distance(closestEnemy.transform.position + new Vector3(closestEnemy.GetComponent<fighterScript>().enemyOffset.x * 1, 
                closestEnemy.GetComponent<fighterScript>().enemyOffset.y, 0),
                transform.position) > Vector3.Distance(closestEnemy.transform.position 
                + new Vector3(closestEnemy.GetComponent<fighterScript>().enemyOffset.x * -1, 
                closestEnemy.GetComponent<fighterScript>().enemyOffset.y, 0), transform.position)) // defines which enemy's side to go
            {
    ...
    void goToEnemy() // go to enemy
    {
        ...
        if (Vector3.Distance(targetPosition, transform.position) > minDistanceBetweenFighters)
        { // checks if fighter is near enemy already
            if (ammunition.Count > 0)
            { // checks if fighter has weapon
                if (!nearEnemy && Mathf.Abs(targetPosition.y - transform.position.y) < 0.5f && Mathf.Abs(targetPosition.x - transform.position.x) <= shootDistance)
                { // checks if 
                    nearEnemy = true;
                    if (targetPosition.x > transform.position.x)
                    { // checks if enemy is right from fighter
                        transform.rotation = Quaternion.Euler(transform.rotation.x, 0, transform.rotation.z);
                        k = 1;
                    }
                    else
                    { // checks if enemy is left from fighter
        ...
        { // checks if enemy is near
            ...
            { // checks if closest enemy is right
                k = 1;
                transform.rotation = Quaternion.Euler(transform.rotation.x, 0, transform.rotation.z);
            }
            else
            { // checks if closest enemy is left
                k = -1;
                transform.rotation = Quaternion.Euler(transform.rotation.x, -180, transform.rotation.z);
            }
    ...
    void checkForFriend() // checks if friends are around
    {
        ...
        if (friends.Length > 1) // checks if there are friends around
        {
            ...
            {
                if (Vector3.Distance(friends[i].gameObject.transform.position, transform.position) < Vector3.Distance(closestFriend.transform.position, transform.position) && friends[i].gameObject != gameObject)
                { // checks if other friend is closer
                    closestFriend = friends[i].gameObject; // stores closest friend
                }
            }
    ...
    void goToFriend() // go to friend
    {
        ...
        if (Vector3.Distance(friendPosition, transform.position) > minDistanceBetweenFriends)
        { // checks if friend is far
    ...
    {
        if (!protectingPosition)
        { // checks if did not change target position yet
            Vector3 direction = patrolPosition - transform.position; // sets direction
            if (Vector3.Distance(patrolPosition, transform.position) > 2)
            { // checks if fighter is not on target position yet
                move(direction.normalized.x, direction.normalized.y); // moves to target position
            }
            else
            { // if on target position
    ...
    void attackEnemy()
    {
        ...
        if (!fighter.dead && !fighter.demobilized)
        { // checks if fighter is alive and not demobilized
            attack();
        }
    ...
    {
        if (grounded && !demobilized && !isBlocking && !dead)
        { // checks if fighter is alive and not demobilized
            grounded = false;
    ...
    {
        if (grounded && !demobilized && !isBlocking && !dead)
        { // checks if fighter is alive, on ground, not demobilized and not blocking
            rb.velocity = Vector2.zero; // stops fighter
            CancelInvoke("run");
            if (ammunition.Count == 0)
            { // if fighter has not weapon
                hit(); // punch
            }
            else
            { // if fighter has weapon
                shoot(); // shoot
            }
        }
    }
    ...
    {
        if (grounded && !demobilized && !isBlocking)
        { // checks if fighter is on ground, not demobilized and not blocking
    ...
    void getHitted(float damage, bool superPunch, int s, Vector2 hitPower) // get hitted 
    {
        ...
        if (!superPunch && health > damage)
        { // receive simple attack
    ...
    public void applyHealth(float plusHealth) // adds plusHealth to health (can be negative)
    {
        ...
        if (health <= 0) // if health is 0 or less
        {
            ...
            if (itemPrefab != null)
            {
                Instantiate(itemPrefab, transform.position, transform.rotation);    // drop item after death
            }
        }
    ...
    public void ComeBackToLife()            // Extra life
    {
        demobilized = false;
        dead = false;
        ...
        gameObject.layer = 8; // sets "ally" layer
    }

    void setHittedMaterial() // sets hitted material
    {
        sr.material = hittedMaterial; // sets hitted material
        Invoke("removeHittedMaterial", 0.05f); // invokes changing hitted material
    }
